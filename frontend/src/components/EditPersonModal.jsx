...
const EditPersonModal = (props) => {
 ...
 const handleSubmit = (e) => {
  ...
  if (data.fnamem.trim() !== '' && data.lnamem.trim() !== '') {
    axios.put('http://localhost:5000/person/edit/'+props.id, data)
    .then(res => {
      console.log(res);
        ...
        <div className='modal-container'>
          <form onSubmit={handleSubmit}>
            <div className='form-group'>
              ...
              <input className='input' type="text" defaultValue={props.fname}
              onChange={e => setData ({...data, fnamem: e.target.value})}/>
            </div>
            <div className='form-group'>
              <label htmlFor="last-name">Last Name*</label>
              <input className='input' type="text" defaultValue={props.lname}
