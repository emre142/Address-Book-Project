...
const EditAddressModal = (props) => {
  const [dataaddress, setDataAddress] = useState({
    addressm: props.address,
    postcodem: props.postcode,
    cidm: props.cid,
    aidm: null,
    coidm: props.coid
});
...
useEffect(() => {
 ...
 const handleSubmit = (e) => {
  e.preventDefault();
  if (dataaddress.postcodem.trim() !== '' && dataaddress.cidm.toString().trim() !== ''
  && dataaddress.addressm.trim() !== '') {
    axios.put('http://localhost:5000/address/edit/'+props.id, dataaddress)
    .then(res => {
      console.log(res);
        ...
        <div className='modal-container'>
            ...
            <div className='form-group'>
              ...
              <select className='select' onChange={e => setDataAddress ({...dataaddress, cidm: e.target.value})}>
                ...
                {Array.isArray(datacity) && datacity.map((city, index) => (
                  city.COUNTRY_ID.toString() === dataaddress.coidm.toString()
          ...
          return <option key={index} value={address_type.ADDRESS_TYPE_ID}>{address_type.ADDRESS_TYPE_NAME}</option>})}
            ...
            <div className='form-group'>
              ...
              <input className='input' type="text" defaultValue={props.address}
              onChange={e => setDataAddress ({...dataaddress, addressm: e.target.value})}/>
            </div>
            <div className='form-group'>
              <label htmlFor="postcode">Postcode*</label>
              <input className='input' type="text" defaultValue={props.postcode}
